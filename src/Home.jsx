  ...
  >
    <tbody>
      {products.reduce((rows, product, index) => {
        ...
        <tr key={rowIndex}>
          {row.map((product) => (
            <td key={product.productId} style={{ verticalAlign: "top", textAlign: "center" }}>
              <div>
              ...
              <div>
                <strong>Prices:</strong> {product.old_price} - {product.new_price}
              </div>
              <div>
                <img
                  ...
                  onClick={() => {
                    document.getElementById(
                      "prod_image"
                    ).innerHTML = `<img id="largeImage" src="https://res.cloudinary.com/dt7a4yl1x/image/upload/c_thumb,w_600/uploads/${product.image_url}" />`;
          ...
          {row.length < 2 && <td></td>} {/* Fill the row with empty cell if needed */}
        </tr>
      ))}
    </tbody>
  </table>
